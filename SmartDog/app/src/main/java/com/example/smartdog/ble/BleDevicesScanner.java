...
public class BleDevicesScanner {
    ...
    private static final long scanPeriod = 20 * 1000; // scan period in milliseconds

    // Data
    private final BluetoothAdapter btAdapter;
    private volatile boolean isScanning = false;
    private Handler handler;
    private List<UUID> servicesToDiscover;
            ...
            new BluetoothAdapter.LeScanCallback() {
                ...
                public void onLeScan(final BluetoothDevice device, final int rssi, final byte[] scanRecord) {
                    synchronized (leScansPoster) {
                        // only process the devices with uuids in servicesToDiscover
                        if (servicesToDiscover == null || !Collections.disjoint(parseUuids(scanRecord), servicesToDiscover)) {
    ...
    public BleDevicesScanner(BluetoothAdapter adapter, UUID[] servicesToDiscover, BluetoothAdapter.LeScanCallback callback) {
        btAdapter = adapter;
        this.servicesToDiscover = servicesToDiscover == null ? null : Arrays.asList(servicesToDiscover);
    ...
    private static class LeScansPoster implements Runnable {
        ...
        void set(BluetoothDevice device, int rssi, byte[] scanRecord) {
            this.device = device;
            this.rssi = rssi;
